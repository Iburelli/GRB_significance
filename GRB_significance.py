        ...
        #----------------------------------------------------Simulation and analysis of VISIBLE sources
                                ...
                                for j in range(60):
                                    ...
                                    if cfg['speed_up']== True:
                                        if night == 'night01' and results[event][site]['night01']['significance']!=-9.0:
                                            if delta_obs > 14400 and '3sigma' not in results[event][site][night].keys() :
                                                break
                                        else:
                                            if results[event][site]['night01']['significance'] == -9.0 and night== 'night02':

                                                if delta_obs > 5400 and '3sigma' not in results[event][site][night].keys() :
                                                    break
                                            if results[event][site]['night01']['significance'] == -9.0 and night== 'night03':
                                                break

                                            if '3sigma' not in results[event][site]['night01'].keys() and results[event][site]['night01']['significance']!=-9.0:
                                               break
